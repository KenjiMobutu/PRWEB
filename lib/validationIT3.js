...
function JVAddOperation() {
	...
	$(function () {
		...
		validation.addField("#savename", [
			{
				validator: function (value) {
					let templateNameAvailable = true;  // Assume the template name is available until proven otherwise
					...
					$.ajax({
						...
						dataType: "json",
						success: (data) => {
							// Check the isAvailable property of the returned data
	...
	// Separate AJAX call into its own function
function checkTemplateName(value, tricId) {
    return new Promise((resolve, reject) => {
        fetch(`templates/validateTemplateNameForIt3/${value}/${tricId}`)
            .then(response => response.json())
            .then(data => {
                if (!data.isAvailable) {
                    // If the name is not available, reject the promise with an error.
                    reject(new Error(
                      "This name is already taken. Please choose a different name."
                    ));
                } else {
                    // If the name is available, resolve the promise.
                    resolve();
                }
            })
            .catch((error) => {
					...
					},
					{
						validator: function (value) {
							...
							$.ajax({
								...
								success: (response) => {
									// Convert the response string to a boolean
									emailExists = response.trim() === "true"; // adjust this based on your server response
								},
